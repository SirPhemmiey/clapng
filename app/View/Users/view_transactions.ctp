    ...
    <table class="table table-hover table-striped table-bordered bootstrap-datatable datatable responsive table-condensed">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?php $isw = $this->requestAction('/ipayments/get_transactions/'.$user['Payment']['trn_ref']); echo $isw['Ipayment']['transaction_reference'] ?></td>
          <td><?php echo $isw['Ipayment']['payment_ref'] ?></td>
          <td><?php echo $isw['Ipayment']['response_code'] ?></td>
          <td><?php echo $isw['Ipayment']['response_desc'] ?></td>
           <td>
             <form  method="post" action="<?php  echo $this->webroot ?>users/requery" >
             <input type="hidden" name="amount" value="123000"/>
             <input type="hidden" name="txnref" value="<?php echo $user['Payment']['trn_ref'];?>" />
             <button type="submit"  class="btn btn-submit" id="makePayment">Requery</button>
             </form>
